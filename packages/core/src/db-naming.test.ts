 ...
 *
 * Test nằm ở `@repo/core` chứ không ở `@repo/db` vì `@repo/db` cố ý không có
 * test runner — nó chỉ chứa schema và Prisma Client. Core là package DUY NHẤT
 * được import Prisma, nên đây là chỗ gần nhất còn chạy được Vitest.
 */
const DB_PACKAGE = path.resolve(import.meta.dirname, "../../db/prisma");
const SCHEMA = path.join(DB_PACKAGE, "schema.prisma");
const MIGRATIONS = path.join(DB_PACKAGE, "migrations");
